...
def load_sovits_model(sovits_path_str_abs, project_root_for_models):
    ...
    if sovits_model_version == "v4":
        ...
        print(f"SoVITS v4 梅尔频谱函数已使用参数: {v4_mel_params} (hps采样率: {hps.data.sampling_rate})")
        ...
        print(f"SoVITS v3 梅尔频谱函数已使用参数: {v3_mel_params} (hps采样率: {hps.data.sampling_rate})")
        ...
        print(f"SoVITS {sovits_model_version} 梅尔频谱函数已使用hps参数 (hps采样率: {hps.data.sampling_rate},num_mels from hps: {num_mels_v1v2}")
...
def perform_tts_on_chunk(ref_wav_path, prompt_semantic_global, phones_prompt_global, bert_prompt_global,
    ...
    # --- 新增：根据sovits_model_version确定声码器目标采样率 ---
    if sovits_model_version == "v4":
        vocoder_target_sr = 32000
        print(f"[打印 perform_tts_on_chunk] SoVITS v4, 声码器目标采样率 vocoder_target_sr: {vocoder_target_sr} Hz")
    elif sovits_model_version == "v3":
        vocoder_target_sr = 24000
        print(f"[打印 perform_tts_on_chunk] SoVITS v3, 声码器目标采样率 vocoder_target_sr: {vocoder_target_sr} Hz")
    else: # v1/v2
        vocoder_target_sr = int(hps.data.sampling_rate)
        print(f"[打印 perform_tts_on_chunk] SoVITS {sovits_model_version}, 目标采样率 vocoder_target_sr (from hps): {vocoder_target_sr} Hz")
    ...
    if sovits_model_version not in {"v3", "v4"}: # SoVITS v1/v2
        ...
        # --- 修改 v1/v2 get_spepc_v1v2 调用 ---
        # librosa.load 的采样率应为 vocoder_target_sr (来自hps)
        audio_ref_v1v2, sr_ref_v1v2_orig = librosa.load(ref_wav_path, sr=None) # 加载原始采样率
        if sr_ref_v1v2_orig != vocoder_target_sr:
            # 如果librosa不支持直接重采样到vocoder_target_sr，或为了与resample函数一致
            # 可以先转tensor再用resample，但get_spepc_v1v2内部的librosa.load通常已处理
            # 此处假设get_spepc_v1v2内部会正确处理或其hps.data.sampling_rate就是vocoder_target_sr
             print(f"[打印 perform_tts_on_chunk] v1/v2: 参考音频原始采样率 {sr_ref_v1v2_orig}, get_spepc_v1v2将使用hps目标采样率 {vocoder_target_sr}")
        ref_spec_v1v2 = get_spepc_v1v2(ref_wav_path) # get_spepc_v1v2 内部使用 hps.data.sampling_rate，现在它应等于 vocoder_target_sr
        # --------------------------------------
        ...
        # --- 修改：使用 vocoder_target_sr 进行重采样 ---
        if ref_sr_orig != vocoder_target_sr:
            print(f"[打印 perform_tts_on_chunk] v3/v4: 参考音频将从 {ref_sr_orig} Hz 重采样到 {vocoder_target_sr} Hz")
            ref_audio_resampled_for_linear = resample(ref_audio_torch_orig, ref_sr_orig, vocoder_target_sr)
            ref_audio_resampled_for_mel = resample(ref_audio_torch_orig, ref_sr_orig, vocoder_target_sr)
        else:
            print(f"[打印 perform_tts_on_chunk] v3/v4: 参考音频原始采样率 {ref_sr_orig} Hz 与目标 {vocoder_target_sr} Hz 一致, 无需重采样。")
        ...
        print(f"[打印 perform_tts_on_chunk] v3/v4: 计算线性频谱使用的采样率: {vocoder_target_sr} Hz, linear_spec_refer shape: {linear_spec_refer.shape}")
        # print(f"[调试 CFM 输入] linear_spec_refer 形状: {linear_spec_refer.shape}") # (B, F_lin, T_frames_ref)
        ...
        ref_mel_for_cfm_raw = mel_fn_dynamic(ref_audio_resampled_for_mel, vocoder_target_sr) # (B, 频点数_梅尔, 帧数_参考)
        print(f"[打印 perform_tts_on_chunk] v3/v4: 计算梅尔频谱的输入音频采样率: {vocoder_target_sr} Hz (mel_fn_dynamic内部固定采样率), ref_mel_for_cfm_raw shape: {ref_mel_for_cfm_raw.shape}")
        ...
        with torch.no_grad():
            ...
            # print(f"[调试 CFM 输入] fea_ref_from_encp 形状: {fea_ref_from_encp.shape}") # (B, C_feat, T_frames_ref_after_encp)
            ...
            # print(f"[调试 CFM 输入] fea_todo_from_encp 形状: {fea_todo_from_encp.shape}") # (B, C_feat, T_frames_target_after_encp)
            ...
            while True:
                ...
                # print(f"[调试 CFM 循环] cfm_res_chunk_mel (CFM输出) 形状: {cfm_res_chunk_mel.shape}") # (B, n_mels, T_combined)
                ...
                # print(f"[调试 CFM 循环] newly_generated_mel_chunk 形状: {newly_generated_mel_chunk.shape}")
    ...
    if audio_output_chunk is not None and audio_output_chunk.size > 0 :
        ...
        print(f"[打印 perform_tts_on_chunk] v3/v4: 声码器输出音频片段长度: {len(audio_output_chunk)} 样本")
...
def get_tts_wav_stitched(ref_wav_path, prompt_text, prompt_language_code,
    ...
    # 返回最终采样率和音频数据
    return hps.data.sampling_rate, final_audio_data
...
def main():
    ...
    # 其他参数
    parser.add_argument("--pause_between_chunks_s", type=float, default=0.3, help="切分文本块之间的静音时长 (秒)")
    parser.add_argument("--project_root_dir", type=str, default=PROJECT_ROOT, help="项目根目录 (用于查找相对路径的预训练模型等)")
    # parser.add_argument("--batch_mode", action="store_true", help="启用批处理模式")
